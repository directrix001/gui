...
class NissanVarianceApp(tk.Tk):
    ...
    def _build_scenario_section(self, parent):
        card = self._section(parent, "03  Forecast Scenarios  (select max 2)")
        card.configure(padx=16, pady=14)
    ...
    def _on_scenario_toggle(self, toggled_scenario):
        ...
        colour = C["success"] if count == 2 else C["silver_dim"]
        self.sc_counter.configure(
            text=f"{count} / 2 selected",
            fg=colour
        )
    ...
    def _clear_scenarios(self):
        for v in self.scenario_vars.values():
            v.set(False)
        self._selected_count = 0
        self.sc_counter.configure(text="0 / 2 selected", fg=C["silver_dim"])
    ...
    def _load_pair(self):
        ...
        self.sc_counter.configure(text="2 / 2 selected", fg=C["success"])
        self.active_pair_label.configure(
            text=f"Active: {label}\n{sc1}  ↔  {sc2}",
            fg=C["success"]
        )
