...
class NissanVarianceApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Right panel (file manifest) - packed first so it reserves space
        right = tk.Frame(main, bg=C["bg_dark"], width=300)
        right.pack(side="right", fill="y", pady=(0, 16))
        right.pack_propagate(False)

        # Left outer container: scrollable form + pinned action bar
        ...
        self._scroll_canvas = tk.Canvas(left_outer, bg=C["bg_dark"], highlightthickness=0)
        vbar = ttk.Scrollbar(left_outer, orient="vertical", command=self._scroll_canvas.yview)
        ...
        self._canvas_window = self._scroll_canvas.create_window((0, 0), window=left, anchor="nw")

        def _on_frame_configure(e):
            self._scroll_canvas.configure(scrollregion=self._scroll_canvas.bbox("all"))
        def _on_canvas_configure(e):
            self._scroll_canvas.itemconfig(self._canvas_window, width=e.width)
        ...
        self._build_output_folder_section(left)   # folder + progress only

        # Pinned action bar - always visible below scroll
    ...
    def _build_scenario_section(self, parent):
        card = self._section(parent, "03  Forecast Scenarios  (select any number  —  each gets MTD + YTD tabs)")
        card.configure(padx=16, pady=14)
    ...
    # ── OUTPUT FOLDER + PROGRESS (inside scroll) ─────────────────
    def _build_output_folder_section(self, parent):
        card = self._section(parent, "04  Output Folder")
        card.configure(padx=16, pady=14)
    ...
    # ── ACTION BAR (pinned outside scroll — always visible) ───────
    def _build_action_bar(self, parent):
        """Two large action buttons pinned at the bottom, always in view."""
    ...
    def _on_scenario_toggle(self, toggled_scenario):
        ...
        if count == 0:
            ...
            colour = C["success"]
            tabs   = count * 2
            label  = f"{count} selected  →  {tabs} tabs (MTD + YTD each)"
    ...
    def _clear_scenarios(self):
        for v in self.scenario_vars.values():
            v.set(False)
        self._selected_count = 0
        self.sc_counter.configure(text="0 selected", fg=C["silver_dim"])
    ...
    def _load_pair(self):
        ...
        self.sc_counter.configure(text="2 selected  →  4 tabs (MTD + YTD each)", fg=C["success"])  # loaded pair always=2
        self.active_pair_label.configure(
            text=f"Active: {label}\n{sc1}  ↔  {sc2}",
            fg=C["success"]
        )
