"""
Nissan Variance Analysis Tool  —  v3.4  (Light UI)
Changes vs v3.3:
  • Template file path is now HARDCODED (see TEMPLATE_FILE near the top);
    the Template picker has been removed from the UI.
  • Read-only master files are handled by the engine (v6.2) — the output
    copy is forced writable so it can actually be edited/saved.
Changes vs v3.2:
  • Light theme — white cards on a soft grey page, Nissan red as the accent.
  • Tidier arrangement and cleaner header.
"""
...
# ─────────────────────────────────────────────
#  THEME / COLOUR PALETTE  (LIGHT)
#  Note: "white" key = primary (dark) text on light surfaces.
#        "on_red"     = light text used on red / coloured surfaces.
# ─────────────────────────────────────────────
C = {
    "bg_dark":    "#F4F5F7",   # page background
    "bg_panel":   "#FAFBFC",   # right panel / status bar
    "bg_card":    "#FFFFFF",   # cards / header
    "bg_input":   "#EDEFF2",   # inputs / listboxes
    "nissan_red": "#C3002F",
    "red_dark":   "#8B0020",
    "red_glow":   "#A80029",   # button hover
    "silver":     "#5C5F66",   # secondary text
    "silver_dim": "#83868C",   # tertiary text / hints
    "white":      "#1B1B1F",   # PRIMARY TEXT (dark)
    "text_dim":   "#9AA0A6",
    "border":     "#E3E6EA",
    "success":    "#1E8E3E",
    "warning":    "#B06000",
    "accent":     "#333333",
    "highlight":  "#FCE8EC",
    "on_red":     "#FFFFFF",   # text on red / coloured fills
}
...
# Combined "Quarter — Month" values for the merged period dropdown
PERIOD_OPTIONS = [
    f"{q.split()[0]}  \u2014  {m}"
    for q, months in MONTHS_BY_Q.items()
    for m in months
]

def _parse_period(value: str):
    """Return (quarter_label, month) from a combined period string."""
    ...
    month   = parts[1].strip()
    quarter = next(
        (q for q in QUARTERS if q.startswith(q_short)), q_short)
...
#  Edit this single line to point at your MTD/YTD template workbook.
#  (The Template picker has been removed from the UI.)
# ─────────────────────────────────────────────
...
def draw_nissan_logo(canvas, cx, cy, scale=1.0):
    ...
    canvas.create_text(cx, cy, text="NISSAN",
                       fill=C["on_red"], font=("Arial", int(9 * scale), "bold"))
...
class NissanVarianceApp(tk.Tk):
    ...
    def _apply_styles(self):
        ...
        style.map("TCombobox",
                  ...
                  bordercolor=[("focus", C["nissan_red"])])

        # Buttons
        style.configure("Red.TButton", background=C["nissan_red"],
                        foreground=C["on_red"],
                        font=("Helvetica Neue", 10, "bold"),
                        borderwidth=0, focusthickness=0, padding=(12, 8))
        style.map("Red.TButton",
                  background=[("active", C["red_glow"]),
                              ("pressed", C["red_dark"])])

        style.configure("Ghost.TButton", background=C["bg_input"],
                        foreground=C["silver"],
                        font=("Helvetica Neue", 10, "bold"),
                        borderwidth=1, focusthickness=0, padding=(12, 8),
                        bordercolor=C["border"], lightcolor=C["bg_input"],
                        darkcolor=C["bg_input"])
        style.map("Ghost.TButton",
                  background=[("active", C["border"]), ("pressed", C["bg_input"])],
                  bordercolor=[("active", C["silver_dim"])])

        style.configure("TProgressbar",
                        troughcolor=C["bg_input"], background=C["nissan_red"],
                        borderwidth=0, thickness=5)

        # Scrollbars — light, slim
        for sb in ("Vertical.TScrollbar", "Horizontal.TScrollbar"):
            style.configure(sb, background=C["border"],
                            troughcolor=C["bg_dark"],
                            bordercolor=C["bg_dark"],
                            arrowcolor=C["silver_dim"])
            style.map(sb, background=[("active", C["silver_dim"])])
    ...
    def _build_header(self):
        hdr = tk.Frame(self, bg=C["bg_card"], height=90)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        tk.Frame(hdr, bg=C["nissan_red"], height=3).pack(fill="x", side="top")
        inner = tk.Frame(hdr, bg=C["bg_card"])
        ...
        tf = tk.Frame(inner, bg=C["bg_card"])
    ...
    def _section(self, parent, title):
        ...
        tk.Frame(lr, bg=C["nissan_red"], width=3, height=14).pack(side="left")
    ...
    def _build_input_section(self, parent):
        ...
        row.pack(fill="x")
        ttk.Button(row, text="Browse  &  Add Folder", style="Red.TButton",
                   command=self._browse_add_scan_folder).pack(side="left")

        self.folder_listbox = tk.Listbox(card, bg=C["bg_input"], fg=C["silver"],
                                          selectbackground=C["nissan_red"],
                                          selectforeground=C["on_red"],
                                          relief="flat", bd=0,
        ...
        self.scan_status = tk.Label(card, text="",
                                     bg=C["bg_card"], fg=C["text_dim"],
    ...
    def _build_period_section(self, parent):
        ...
        tk.Label(card, text="Quarter  &  Month", bg=C["bg_card"],
                 fg=C["silver_dim"], font=("Helvetica Neue", 8)).pack(
                     anchor="w", pady=(0, 4))
        self.period_var = tk.StringVar()
        self.period_cb  = ttk.Combobox(card, textvariable=self.period_var,
                                        values=PERIOD_OPTIONS, state="readonly",
                                        font=("Helvetica Neue", 10))
        self.period_cb.pack(fill="x", ipady=4)
    ...
    def _build_scenario_section(self, parent):
        ...
        self.pairs_listbox = tk.Listbox(
            ...
            selectbackground=C["nissan_red"], selectforeground=C["on_red"],
            relief="flat", bd=0, highlightthickness=1,
            highlightbackground=C["border"],
            font=("Courier", 8), height=8, activestyle="none")
    ...
    def _build_master_file_section(self, parent):
        ...
        # Fixed template indicator (template picker removed from UI)
        ok = os.path.isfile(TEMPLATE_FILE)
        tk.Label(card,
                 text=f"Template (fixed): {os.path.basename(TEMPLATE_FILE)}"
                      + ("" if ok else "   — NOT FOUND, edit TEMPLATE_FILE in the script"),
                 bg=C["bg_card"], fg=(C["silver_dim"] if ok else C["warning"]),
                 font=("Helvetica Neue", 8, "italic")).pack(anchor="w", pady=(2, 0))

    # ── 05  OUTPUT FOLDER + PROGRESS ────────────────────────────
    def _build_output_folder_section(self, parent):
        card = self._section(parent, "05  Output Folder")
        card.configure(padx=16, pady=14)
    ...
    def _build_statusbar(self):
        ...
        tk.Label(bar, textvariable=self.status_var,
                 bg=C["bg_panel"], fg=C["silver_dim"],
                 font=("Helvetica Neue", 8)).pack(side="left", padx=12, pady=4)
        tk.Label(bar, text="v3.4  -  Nissan FP&A",
    ...
    def _generate_file(self):
        if not self.period_var.get():
            messagebox.showwarning("No Period", "Please select a Financial Period.")
            return

        quarter, month = _parse_period(self.period_var.get())
        if not quarter or not month:
            messagebox.showwarning("Invalid Period", "Could not parse the selected period.")
            return
        ...
        template = TEMPLATE_FILE.strip()
        if not template or not os.path.isfile(template):
            messagebox.showwarning(
                "Template Not Found",
                "The hardcoded template file was not found:\n\n"
                f"{TEMPLATE_FILE}\n\n"
                "Edit the TEMPLATE_FILE path near the top of the script.")
            return
    ...
    #  FOLDER / FILE LOGIC
    # ══════════════════════════════════════════════════════════════
    def _browse_add_scan_folder(self):
        """Single button: opens dialog → adds folder → scans immediately."""
        ...
        self.status_var.set(f"Folder added — scanning: {os.path.basename(path)}")
    ...
    def _on_scan_complete(self):
        ...
        msg = f"Scan complete — {total} file(s) across {len(self.input_folders)} folder(s)"
        self.status_var.set(msg)
        try:
            self.scan_status.configure(text=msg, fg=C["success"])
        except Exception:
            pass
