...
class NissanVarianceApp(tk.Tk):
    def __init__(self):
        ...
        # Exactly 2 scenario slots
        self.scenario_vars: dict = {}
        self._selected_count = 0

        # Master file path (selected by user at runtime)
    ...
    def _build_ui(self):
        ...
        # Right panel (file manifest) — packed first to reserve space
    ...
    # ══════════════════════════════════════════════════════════════
    #  03  SCENARIO SECTION  — exactly 2 = 1 comparison pair
    # ══════════════════════════════════════════════════════════════
    def _build_scenario_section(self, parent):
        card = self._section(parent, "03  Forecast Scenarios  (select exactly 2  -  1 comparison pair  ->  MTD + YTD tabs)")
        card.configure(padx=16, pady=14)
        ...
        # Counter badge shows the live pair
        self.sc_counter = tk.Label(left_col,
                                    text="Select 2 scenarios to form a comparison pair",
    ...
    def _build_master_file_section(self, parent):
        card = self._section(parent, "04  Master Input File  (contains data to populate tabs)")
        card.configure(padx=16, pady=14)
    ...
    def _build_action_bar(self, parent):
        ...
        btn_row = tk.Frame(bar, bg=C["bg_dark"])
        btn_row.pack(fill="x")
    ...
    def _build_statusbar(self):
        ...
        tk.Label(bar, text="v2.1  -  Nissan FP&A",
                 bg=C["bg_panel"], fg=C["text_dim"],
                 font=("Helvetica Neue", 8)).pack(side="right", padx=12, pady=4)

    # ══════════════════════════════════════════════════════════════
    #  SCENARIO LOGIC — enforce exactly 2
    # ══════════════════════════════════════════════════════════════
    def _on_scenario_toggle(self, toggled_scenario):
        ...
        if count > 2:
            # Uncheck the one just toggled to keep max at 2
            self.scenario_vars[toggled_scenario].set(False)
            selected = [s for s, v in self.scenario_vars.items() if v.get()]
            count = 2
            self.status_var.set("Exactly 2 scenarios allowed. Deselect one first.")
        ...
        if count == 0:
            ...
            self.sc_counter.configure(
                text=f"1 of 2:  {selected[0]}  — select one more",
                fg=C["warning"])
        else:
            pair_label = f"{selected[0]}  vs  {selected[1]}"
            self.sc_counter.configure(
                text=f"Pair ready:  {pair_label}  ->  MTD + YTD tabs",
                fg=C["success"])
    ...
    def _load_pair(self):
        ...
        self.sc_counter.configure(
            text=f"Pair ready:  {sc1}  vs  {sc2}  ->  MTD + YTD tabs",
            fg=C["success"])
        self.active_pair_label.configure(
            text=f"Active: {label}\n{sc1}  <->  {sc2}",
            fg=C["success"]
    ...
    def _run_analysis(self):
        ...
        if len(selected) != 2:
            messagebox.showwarning("Select 2 Scenarios",
                                   "Please select exactly 2 scenarios to run the analysis.")
            return
        ...
        if not master or not os.path.isfile(master):
            messagebox.showwarning("No Master File",
                                   "Please select a valid Master Input File in section 04.")
            return
        ...
        confirm = (
            f"Run Variance Analysis:\n\n"
            f"  Scenario 1  :  {selected[0]}\n"
            f"  Scenario 2  :  {selected[1]}\n"
            f"  Quarter     :  {self.quarter_var.get()}\n"
            f"  Month       :  {self.month_var.get()}\n"
            f"  Master file :  {os.path.basename(master)}\n"
            f"  Output      :  {out_dir}\n\n"
            f"Output will have 2 tabs:\n"
            f"  MTD {selected[0]} vs {selected[1]}\n"
            f"  YTD {selected[0]} vs {selected[1]}\n\n"
            f"Proceed?"
        )
    ...
    def _run_thread(self, args: dict):
        try:
            ...
            importlib_reload = getattr(__import__("importlib"), "reload", None)
            if importlib_reload:
                importlib_reload(variance_engine)
            self.after(0, self._update_progress, 50, "Building output workbook...")
            out_path = variance_engine.run_variance(args)
            self.after(0, self._on_run_complete, args["output_folder"], out_path, None)
        except ImportError:
            self.after(0, self._on_run_complete, args["output_folder"], None,
                       "variance_engine.py not found in the same folder.")
        except Exception as e:
            self.after(0, self._on_run_complete, args["output_folder"], None, str(e))

    def _on_run_complete(self, out_dir, out_path, error):
        ...
        if error:
            ...
            self.progress_label.configure(text=f"Analysis complete  {fname}")
            self.status_var.set(f"Saved: {out_path}")
            if messagebox.askyesno("Done", f"Analysis complete.\n\n{out_path}\n\nOpen output folder?"):
    ...
    def _generate_files(self):
        ...
        if len(selected) != 2:
            messagebox.showwarning("Select 2 Scenarios",
                                   "Please select exactly 2 scenarios.")
            return
        out_dir = self.output_entry.get().strip()
        if not out_dir or out_dir == "Select output folder...":
            messagebox.showwarning("No Output Folder", "Please select an output folder.")
        ...
        confirm_msg = (
            ...
            f"  Pair       : {selected[0]} vs {selected[1]}\n"
            f"  Quarter    : {self.quarter_var.get()}\n"
            f"  Month      : {self.month_var.get()}\n\n"
    ...
    def _generate_thread(self, out_dir, scenarios):
        total_steps = max(sum(len(v) for v in self.files_found.values()), 1)
        step = 0
        quarter = self.quarter_var.get().split()[0]
        month = self.month_var.get()
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        sc_label = f"{scenarios[0].replace(' ','_').replace('+','p')}_vs_{scenarios[1].replace(' ','_').replace('+','p')}"
        ...
        for folder, files in self.files_found.items():
            for fname in files:
                ...
                self.after(0, self._update_progress, pct, f"Processing {fname}")
                time.sleep(0.05)
    ...
    def _browse_master_file(self):
        path = filedialog.askopenfilename(
            title="Select Master Input File",
    ...
    def _show_loaded_popup(self, total):
        ...
        tk.Label(popup, text=f"{total} file(s) from {len(self.input_folders)} folder(s).",
                 bg=C["bg_card"], fg=C["silver_dim"],
