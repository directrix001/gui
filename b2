"""
variance_engine.py  —  v6.0  (DEFINITIVE)
==========================================
Guaranteed no broken links / no workbook repair dialog.

Every root cause found by live XML inspection and fixed:

BUG-1  Absolute target paths with leading "/"
        openpyxl writes Target="/xl/worksheets/sheet2.xml".
        os.path.join(base, "xl", "/xl/...") drops base entirely
        because the second argument is absolute.
        FIX: strip leading "/" with lstrip("/") before joining.

BUG-2  Injected <sheet> elements missing xmlns:r declaration
        The workbook root only declares the default namespace.
        Each <sheet r:id="..."> uses the "r:" prefix which must be
        declared on the element itself (or an ancestor).
        Without it Excel sees an undeclared namespace → XML error → repair.
        FIX: ensure workbook.xml root element carries
             xmlns:r="...officeDocument/2006/relationships"
             (added once to the root, covers all child <sheet> elements).

BUG-3  openpyxl.save() called AFTER zip surgery destroys all patches
        After zip surgery, calling load_workbook(output)+wb.save()
        makes openpyxl completely rewrite workbook.xml, workbook.xml.rels
        and [Content_Types].xml — undoing every patch we made and
        re-introducing external link records.
        FIX: audit log sheet is generated as raw XML and injected into
             the zip directly.  openpyxl.save() is NEVER called on the
             output file in the zip path.

BUG-4  sharedStrings.xml absent in output but referenced by copied sheets
        If master has no sharedStrings and template does, copied sheets
        reference string indices that don't exist → repair drops all text.
        FIX: copy sharedStrings.xml from template into output and
             register it in Content_Types + workbook.xml.rels.

BUG-5  styles.xml index mismatch
        Copied sheets use styleIndex values from the template's styles.xml.
        If the master has fewer style entries those indices are out-of-range.
...
def _ensure_r_namespace_on_root(out_dir: str):
    ...
    Without this, any <sheet r:id="..."> element uses an undeclared
    namespace prefix and Excel refuses to open the file.
    """
    path = os.path.join(out_dir, "xl", "workbook.xml")
    text = _rt(path)
    if 'xmlns:r=' in text:
        return   # already declared
    # Insert xmlns:r into the opening <workbook ...> tag
    text = re.sub(
        r'(<workbook\b)',
        r'\1 xmlns:r="' + _NS_R + '"',
        text, count=1)
...
def _sheet_map(base_dir: str) -> dict:
    ...
    Handles both attribute orderings and both absolute/relative targets.
    """
...
def _wb_add_sheet(out_dir: str, name: str, rid: str):
    ...
    safe = (name.replace("&","&amp;").replace('"',"&quot;")
                .replace("<","&lt;").replace(">","&gt;"))
    # Include xmlns:r on the element as a safety net
...
def _strip_ext_links(ws_path: str, tpl_base: str, tpl_base_nx: str):
    ...
    Operates on raw text — no XML re-serialisation.
    """
    pats = [
...
def _purge_ext_refs(out_dir: str, tpl_base: str, tpl_base_nx: str):
    ...
    Remove all external-link artefacts:
      • <externalReferences> block from workbook.xml
      • externalLink <Relationship> entries from workbook.xml.rels
      • xl/externalLinks/ folder
      • Any <definedName> referencing the template
    """
...
def _inject_audit(out_dir: str, args: dict, pairs: list,
    ...
    """
    Build 'Input Log' sheet as minimal worksheet XML and inject it into
    the zip.  openpyxl.save() is NEVER called on the output workbook.
    """
    rows: list[list[str]] = []
